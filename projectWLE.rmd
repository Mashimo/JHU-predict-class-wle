...
First check if file exists (assume the folder with the data sets is in the same directory as this script)
```{r}
...
This is not really necessary bu I like to rename the first column from "X" to a meaningful name

```{r}
colnames(wleTrain)[1] <- "ID"  
```

# 1. do some exploratory data analyses = look for outliers, skewness, imbalances in outcome/predictors
```{r, echo=FALSE}
summary(wleTrain)
library(ggplot2)
...
First of all, it's clear from the summary above that the first six variables have no values.
```{r}
library(caret)
...
From the summary above, some variables have missing values. We could substitute them with the average for
that variable but there are many missing values and this is not improving the model accuracy (I tried).
Instead of less-accurate imputation of missing data, I just remove all variables with NA values.
Hard but fair ...
...
# 3. split the training set to have a validation set for all the candidate models (Cross-validation)
...
# 4. train all competing models on the training data set using the CV folds method
...
accuracy <- sum(diag(confusion)) / nrow(validation)   # 0.56
```
Accuracy is 0.56 (56% of the times is right) that is better than baseline.
But LDA can also be used to reduce dimensions (like PCA but preserve the class discriminatory information). 
In this case it returns 4 Linear Discriminants (instead of  the initial 30 predictors)

## 4.2 decision tree model
Train the model on all predictors using cross-validation.

```{r}
modTree <- train(classe ~ ., method="rpart", trControl = cvControl, data= training)
print(modTree$finalModel)
```
Trees are easy to interpret; the trained model uses this 6 predictors: roll_dumbbell, pitch_forearm, magnet_belt_z, roll_forearm, total_accel_dumbbell and gyros_belt_z
...
confusion <- table(validation$classe, predictions)
accuracy <- sum(diag(confusion)) / nrow(validation)   # 0.50
```
...
confusion <- table(validation$classe, predictions)
accuracy <- sum(diag(confusion)) / nrow(validation)
```
Accuracy is 0.91, much higher

# 4.4 random forest (= many trees :)
